...
inductive Ref where | NullPtr | Ptr (n : Nat)
    deriving ToJson, Repr, BEq


def Ref.toNat (r : Ref) : Option Nat := 
    match r with
    | .NullPtr => none 
    | .Ptr v => some v

instance : ToString Ref where 
    toString := reprStr ∘ Ref.toNat
...
abbrev ValueEnum := ValueEnumA Int
...
structure  BytecodeValueA (α : Type*) where
     value : ValueEnumA α
     deriving ToJson, Repr, BEq

abbrev BytecodeValue := BytecodeValueA Int
...
def BytecodeValueFromJson (j : Json) : Except String BytecodeValue := do
    ...
    |(KindEnum.KindInt, .ValInt i) => pure ⟨.ValInt i⟩
...
inductive Operation where
  ...
  | Cast (offset : Nat) («from» : KindEnum) (toKind : KindEnum)
...
def CastFromJson (j : Json) : Except String Operation := do
  ...
  let toKind ← fromJson? (← j.getObjVal? "to")
  return .Cast offset «from» toKind
...
info: Except.ok { annotations := #[],
  bytecode := #[Operation.Push 0 (some ValueEnumA.ValInt 1)],
  exceptions := #[],
...
/-- info: Except.ok ValueEnumA.ValInt 1 -/
#guard_msgs in
...
/-- info: Except.ok ValueEnumA.ValClass { kind := KindEnum.Class, name := "jpamb/cases/Simple" } -/
#guard_msgs in
...
/--
info: Except.ok (Operation.Push 0 (some ValueEnumA.ValClass { kind := KindEnum.Class, name := "jpamb/cases/Simple" }))
-/
#guard_msgs in
